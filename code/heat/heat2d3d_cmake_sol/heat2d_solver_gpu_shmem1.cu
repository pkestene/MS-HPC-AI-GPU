...
runTest(int argc, char** argv) 
{
  ...
  for (iTime=0; iTime*DT<TMAX; ++iTime) {
    ...
    if (useOrder2) { // use the 2nd order accurate scheme
      ...
      heat2d_ftcs_sharedmem_order2_kernel<<< grid, threads >>>( data_in, data_out,
								NX, NY);
      ...
      heat2d_ftcs_sharedmem_order2b_kernel<<< grid, threads >>>( data_in, data_out,
								 NX, NY);
      CUDA_KERNEL_CHECK("Kernel execution failed");
      
    } else { // use the 4th order accurate scheme
      
      heat2d_ftcs_sharedmem_order4_kernel<<< grid, threads >>>( data_in, data_out, 
								NX, NY);
      CUDA_KERNEL_CHECK("Kernel execution failed");
