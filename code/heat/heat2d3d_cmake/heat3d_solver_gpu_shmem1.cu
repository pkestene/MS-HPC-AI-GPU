/**
 * \file heat3d_solver_gpu_shmem1.cu
 * \brief Solve 3D heat equation (finite difference method). GPU version (shmem1).
 *
...
// GPU solver
#include "heat3d_kernel_gpu_shmem1.cu"
...
runTest(int argc, char** argv) 
{
  ...
  printParameters("HEAT 3D - GPU (SHMEM1)");
  ...
  for (iTime=0; iTime*DT<TMAX; ++iTime) {
    ...
    if (useOrder2) { // use the 2nd order accurate scheme
      ...
      heat3d_ftcs_sharedmem_order2_kernel<<< grid, threads >>>( data_in,
								data_out,
								NX, NY, NZ);
      ...
      heat3d_ftcs_sharedmem_order4_kernel<<< grid, threads >>>( data_in, 
								data_out,
								NX, NY, NZ);
      CUDA_KERNEL_CHECK("Kernel execution failed");
	...
	save_vtk(data1, "heat3d_gpu_shmem1_",iOutput);

      // HDF5 output
      if (SAVE_HDF5 and iTime%T_OUTPUT == 0)
	save_hdf5(data1, "heat3d_gpu_shmem1_",iOutput);
  ...
  if (SAVE_HDF5)
    write_xdmf_wrapper("heat3d_gpu_shmem1",N_ITER,T_OUTPUT);
